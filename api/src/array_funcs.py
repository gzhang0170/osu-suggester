...
class ArrayFuncs:
    ...
    """
    def get_num_map_stats(self, max_limit=1000):
        index = 0
        failed = 0

        # TODO: Change this to the correct path using os
        folder_path = "../2024_12_01_osu_files"
        #file_count = len([f for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f))])
        
        MAX_TABLE_SIZE = max_limit
        NUM_ATTRIBUTES = 15
        ...
        for file_name in os.listdir(folder_path):
            ...
            map_table[index][0] = int(file_name.replace(".osu", ""))
            map_table[index][1] = diff_attrs.stars
            map_table[index][2] = map.bpm
            map_table[index][3] = map.cs
            map_table[index][4] = map.ar
            map_table[index][5] = map.od
            map_table[index][6] = map.hp
            ...
            map_table[index][14] = int(diff_attrs.mode)

            index += 1
            print(f"Progress: {index} / {MAX_TABLE_SIZE}, {(index / MAX_TABLE_SIZE * 100):2f}%            ", end="\r")
            if index >= MAX_TABLE_SIZE:
                print(f"Success: {index - failed}, Failed: {failed}, {(((MAX_TABLE_SIZE - failed) / MAX_TABLE_SIZE) * 100):2f}%")
                break
    ...
    def get_data_stats(self, map_table):
        ...
        for map_stats in map_table:
            if map_stats[0] != 0 and map_stats[14] == 0:
    ...
    """
    def find_most_similar(self, data_table, ref_index, top_n=10):
